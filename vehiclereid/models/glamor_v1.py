...
class GLAMOR(nn.Module):
    def __init__(self, num_classes, loss, block, layers,
        ...
        self.fc = self._construct_fc_layer(fc_dims, 512 * 4, dropout_p)
    ...
    def featuremaps(self, x):
        ...
        l = self.CBAM(g)
        self.mask_0 = torch.zeros(x.shape[0], 128, 1, 1).cuda()
        self.mask_1 = torch.ones(x.shape[0], 128, 1, 1).cuda()
        self.mask = torch.cat((self.mask_0, self.mask_1), 1).cuda()
        self.nmask = torch.cat((self.mask_1, self.mask_0), 1).cuda()
        f = g * self.mask + l * self.nmask
...
def glamor50_v1(num_classes, loss = {'xent', 'htri'}, **kwargs):
    model = GLAMOR(
