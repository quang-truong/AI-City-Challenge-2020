...
class GLAMOR(nn.Module):
    ...
    def featuremaps(self, x):
        ...
        l = self.LAM(g)
        mask_0 = torch.zeros(x.shape[0], 128, 1, 1).cuda()
        mask_1 = torch.ones(x.shape[0], 128, 1, 1).cuda()
...
def glamor50(num_classes, loss = {'xent', 'htri', 'center'}, **kwargs):
    model = GLAMOR(
...
def glamor18(num_classes, loss = {'xent', 'htri', 'center'}, **kwargs):
    model = GLAMOR(
        num_classes = num_classes,
        loss = loss,
        block = Bottleneck,
