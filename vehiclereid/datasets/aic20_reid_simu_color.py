...
class AIC20_ReID_Simu_Color(BaseImageDataset):
    dataset_dir = 'AIC20_ReID_Simulation'

    def __init__(self, root='datasets', verbose=True, **kwargs):
        super((AIC20_ReID_Simu_Color), self).__init__(root)
        ...
        if verbose:
            print('=> AIC20_ReID_Simu_Color loaded')
        ...
        for i in itemlist:
            if (i.attributes['vehicleID'].value in train_container):
                train_data.append((
                    ...
                    int(i.attributes['colorID'].value), 
