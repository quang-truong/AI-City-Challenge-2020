...
class AIC20_ReID_Type(BaseImageDataset):
    """
    AIC20_ReID
    ...
    def __init__(self, root='datasets', verbose=True, **kwargs):
        super((AIC20_ReID_Type), self).__init__(root)
        ...
        with open("train_pid2type.pickle", "rb") as f:
            self.pid2type = pickle.load(f)
    ...
    def process_dir(self):
        pid_dict = {0:0, 3:0, 8:0, 9:0, 1:1, 4:1, 2:2, 5:3, 7:4, 6:5, 10:5}
        ...
        query_dict = {}
        for i in itemlist:
            if (pid_dict[self.pid2type[i.attributes['imageName'].value]] not in query_dict):
                query_dict[pid_dict[self.pid2type[i.attributes['imageName'].value]]] = 1
                query_data.append((
                    osp.join(img_path, i.attributes['imageName'].value), 
                    pid_dict[self.pid2type[i.attributes['imageName'].value]], 
                    int(i.attributes['cameraID'].value[1:])))
            else:
                if (pid_dict[self.pid2type[i.attributes['imageName'].value]] not in test_dict):
                    test_dict[pid_dict[self.pid2type[i.attributes['imageName'].value]]] = 1
                    gallery_data.append((
                        osp.join(img_path, i.attributes['imageName'].value), 
                        pid_dict[self.pid2type[i.attributes['imageName'].value]], 
                        int(i.attributes['cameraID'].value[1:])))
                elif (test_dict[pid_dict[self.pid2type[i.attributes['imageName'].value]]] <= 30):
                    test_dict[pid_dict[self.pid2type[i.attributes['imageName'].value]]] += 1
                    gallery_data.append(
                        (osp.join(img_path, i.attributes['imageName'].value), 
                        pid_dict[self.pid2type[i.attributes['imageName'].value]], 
